...
public class AsyncConfig {
    ...
    private static final ModConfigSpec.ConfigValue<
        List<? extends String>
    ...
    private static final ModConfigSpec.BooleanValue skipErroringEntities;

    static {
        BUILDER.push("Async Config");

        disabled = BUILDER.comment(
            "Enables parallel processing of entities."
        ).define(
            "disabled",
            com.axalotl.async.common.config.AsyncConfig.disabled
        );
        ...
        maxThreads = BUILDER.comment(
            ...
            "maxThreads",
            com.axalotl.async.common.config.AsyncConfig.maxThreads,
            -1,
            Integer.MAX_VALUE
        );
        ...
        synchronizedEntities = BUILDER.comment(
            ...
            () ->
                new ArrayList<>(
                    com.axalotl.async.common.config.AsyncConfig.synchronizedEntities
                ),
            () -> "",
            obj -> obj instanceof String
        );

        enableAsyncSpawn = BUILDER.comment(
            "Enables async entity spawning. WARNING: incompatible with Carpet's lagFreeSpawning."
        ).define(
            "enableAsyncSpawn",
            com.axalotl.async.common.config.AsyncConfig.enableAsyncSpawn
        );

        enableAsyncRandomTicks = BUILDER.comment(
            "Experimental! Enables async random ticks."
        ).define(
            "enableAsyncRandomTicks",
            com.axalotl.async.common.config.AsyncConfig.enableAsyncRandomTicks
        );

        skipErroringEntities = BUILDER.comment(
            "Skip entities that error during synchronous ticking instead of crashing."
        ).define(
            "skipErroringEntities",
            com.axalotl.async.common.config.AsyncConfig.skipErroringEntities
        );
    ...
    public static void loadConfig() {
        ...
        com.axalotl.async.common.config.AsyncConfig.maxThreads =
            maxThreads.get();
        com.axalotl.async.common.config.AsyncConfig.enableAsyncSpawn =
            enableAsyncSpawn.get();
        com.axalotl.async.common.config.AsyncConfig.enableAsyncRandomTicks =
            enableAsyncRandomTicks.get();
        com.axalotl.async.common.config.AsyncConfig.skipErroringEntities =
            skipErroringEntities.get();
        ...
        com.axalotl.async.common.config.AsyncConfig.synchronizedEntities =
            entities.isEmpty() ? getDefaultSynchronizedEntities() : entities;
    }
    ...
    public static void saveConfig() {
        ...
        enableAsyncSpawn.set(
            com.axalotl.async.common.config.AsyncConfig.enableAsyncSpawn
        );
        enableAsyncRandomTicks.set(
            com.axalotl.async.common.config.AsyncConfig.enableAsyncRandomTicks
        );
        skipErroringEntities.set(
            com.axalotl.async.common.config.AsyncConfig.skipErroringEntities
        );
        synchronizedEntities.set(
            new ArrayList<>(
                com.axalotl.async.common.config.AsyncConfig.synchronizedEntities
            )
        );
